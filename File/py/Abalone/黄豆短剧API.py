...
class Spider(BaseSpider):
    def __init__(self):
        ...
        self.platform_keys = [
            ...
            "c10ca2986a31fb46d4481ce8631c2725",
        ]
    ...
    def _api(self, path, data=None, silent=False):
        ...
        if good:
            if good[0] in hosts:
                hosts.remove(good[0])
                hosts.insert(0, good[0])
            if good[1] in keys:
                keys.remove(good[1])
                keys.insert(0, good[1])
        tried, n = 0, 0
        for host in hosts:
            for pk in keys:
                tried += 1
    ...
    def _post(self, host, path, payload, pk_str):
        rid = str(uuid.uuid4())
        iv = os.urandom(16)
        raw = json.dumps({"token": self.token or "", "deviceId": self.device_id, "data": payload}, ensure_ascii=False, separators=(",", ":")).encode("utf-8")
        body = iv + _AESCBC.encrypt(gzip.compress(raw), self._key(rid, pk_str), iv)
        ts = int(time.time())
        sign = hashlib.sha256(("Dart|%s|%s|%s|%s" % (self.session_id, rid, ts, path)).encode("utf-8")).hexdigest() + "-" + str(ts)
        h = dict(self.headers)
        h.update({"version": self.version, "deviceType": self.device_type, "time": str(ts), "sign": sign, "requestId": rid, "sessionId": self.session_id, "deviceBrand": "", "deviceModel": "", "systemName": "", "systemVersion": "", "Origin": host, "Referer": host + "/home"})
        try:
            r = self.session.post(host + "/api" + path, data=body, headers=h, timeout=12, verify=False)
            r.raise_for_status()
        except Exception:
            return None
        try:
            return self._decode(r.content, rid, pk_str)
        except Exception:
            return None

    def _key(self, rid, pk_str=None):
        k = (pk_str or self.platform_key).encode("utf-8")
        return hmac.new(k, bytes.fromhex(str(rid).replace("-", "")), hashlib.sha256).digest()

    def _decode(self, blob, rid, pk_str=None):
        ...
        for ks in keys:
            try:
                plain = _AESCBC.decrypt(blob[16:], self._key(rid, ks), blob[:16])
                if plain[:2] == b"\x1f\x8b":
                    plain = gzip.decompress(plain)
                return json.loads(plain.decode("utf-8"))
            except Exception as e:
                last = e
        raise last or ValueError("decrypt failed")
