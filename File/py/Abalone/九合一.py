...
class Spider(SpiderBase):
    ...
    def _extract_remarks(self, raw_block):
        remarks = ""
        for cls_pat in ("remarks", "label", "tag", "pic-text", "des", "text-gray", "info", "tip", "state"):
            m = re.search(r'class=["\'][^"\']*' + cls_pat + r'[^"\']*["\'][^>]*>([^<]+)', raw_block, re.I)
            if m:
                remarks = m.group(1).strip()
                break
        if not remarks:
            m = re.search(r'<[^>]*class=["\'][^"\']*text[^"\']*["\'][^>]*>([^<]{1,20})', raw_block, re.I)
            if m: remarks = m.group(1).strip()
        return remarks

    def _build_detail_url(self, play_url, host):
        if not play_url:
            return ""
        ...
        if play_url.startswith("/"):
            return host + play_url
        if play_url.startswith("z1_mk://"):
            mk_id = play_url.replace("z1_mk://", "").strip()
            return "%s/html/%s.html" % (host, mk_id)
        if "." in play_url:
            return host + ("/" if not play_url.startswith("/") else "") + play_url
        return "%s/html/%s.html" % (host, play_url)

    def _parse_detail(self, html_text, fallback_title=""):
        actor = ""
        director = ""
        content = ""
        remarks = ""
        if not html_text:
            return actor, director, content, remarks
        
        # 导演
        m = re.search(r'导演[：:\s]*([^<\n]+)', html_text)
        if m: director = m.group(1).strip()
        if not director:
            m = re.search(r'(?i)vod_director[^>]*>([^<]+)', html_text)
            if m: director = m.group(1).strip()
        ...
        if not actor:
            m = re.search(r'(?i)vod_actor[^>]*>([^<]+)', html_text)
            if m: actor = m.group(1).strip()
        
        # 简介
        m = re.search(r'(?i)vod_content[^>]*>([\s\S]*?)</(?:div|p|span)>', html_text)
        if m:
            content = re.sub(r'<[^>]+>', '', m.group(1)).strip()
        if not content:
            m = re.search(r'(?i)(简介[：:\s]*)([\s\S]{0,800}?)(?:\n\n|<div\s|</div>\s*</div>)', html_text)
            if m: content = m.group(2).strip()
        
        # 备注/更新状态
        m = re.search(r'(?i)vod_remarks[^>]*>([^<]+)', html_text)
        if m: remarks = m.group(1).strip()
        ...
        for raw_block in matches:
            try:
                ...
                vod_list.append({
                    ...
                    "vod_name": title,
                    "vod_pic": proxy_pic,
                    "vod_remarks": self._extract_remarks(raw_block),
    ...
    def detailContent(self, ids):
        ...
        detail_url = self._build_detail_url(play_url, host)
        actor, director, content, remarks = "", "", "", ""
        if detail_url:
            res = self._fetch(detail_url, host)
            html_text = res.get("text", "")
            if html_text:
                actor, director, content, remarks = self._parse_detail(html_text, title)
